...
for(i in seq(1,length(parts),by=1)){
    ...
    #append seat ids to vals
    vals <- c(vals,8*temp_rows + temp_seats)
}
#sort the list to find any differences between adjacent numbers >1
vals <- sort(vals)
x <- vals[1:length(vals)-1] - vals[2:length(vals)]
#print the seats around the culprit
cat(vals[match(-2, x) -1], vals[match(-2, x)], vals[match(-2, x) +1] )
